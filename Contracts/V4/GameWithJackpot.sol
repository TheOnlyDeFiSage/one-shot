...
contract GameWithJackpot is Ownable, ReentrancyGuard {
    ...
    // New variables for tracking betting history
    struct BetInfo {
        uint256 amount;         // Bet amount
        uint256 timestamp;      // When the bet was placed
        bool won;               // Whether the bet was won
        uint256 payout;         // Amount paid out (0 if lost)
        uint256 jackpotWon;     // Jackpot amount won (0 if none)
    }
    
    // Tracking last 10 bets for each user (circular buffer)
    mapping(address => BetInfo[10]) private userBetHistory;
    mapping(address => uint8) private userBetIndex;
    ...
    function play() external payable nonReentrant {
        ...
        if (won) {
            ...
            // Increase win streak and update total wins
            winStreaks[msg.sender] = winStreaks[msg.sender].add(1);
            totalWins[msg.sender] = totalWins[msg.sender].add(1);
            
            uint256 currentStreak = winStreaks[msg.sender];
            
            // Check if player achieves any jackpot tier
            (bool jackpotWon, uint256 tierIndex, uint256 wonJackpotAmount) = checkJackpotWin(msg.sender);
            jackpotAmount = wonJackpotAmount; // Store for bet history
            ...
            // Reset win streak and update total losses
            winStreaks[msg.sender] = 0;
            totalLosses[msg.sender] = totalLosses[msg.sender].add(1);
        ...
        // Record bet history
        uint8 currentIndex = userBetIndex[msg.sender];
        userBetHistory[msg.sender][currentIndex] = BetInfo({
            amount: betAmount,
        ...
        // Update index for circular buffer (overwrite oldest entries)
        userBetIndex[msg.sender] = uint8((uint256(currentIndex) + 1) % 10);
     ...
     * @return The name of the tier (e.g., "Tier 1 (4-Win)", "Tier 2 (6-Win)", etc.)
     */
    ...
    /**
     * @dev Get betting history for a player
     * @param player The address of the player
     * @return The last 10 bets (or fewer if player hasn't made 10 bets)
     */
    function getBetHistory(address player) external view returns (BetInfo[10] memory) {
        return userBetHistory[player];
    }
    
    /**
     * @dev Get player statistics (wins, losses, current streak)
     * @param player The address of the player
     * @return wins Total number of wins
     * @return losses Total number of losses
     * @return currentStreak Current win streak
     */
